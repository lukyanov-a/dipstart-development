<?php

return array(
    'yes' => 'да',
	'no' => 'нет',
    'Fields lists' => 'Списки полей',
    'Create Categories' => 'Добавить категории',
    'Manage Categories' => 'Управлять списком',
    'Home' => 'Главная',
    'Users' => 'Пользователи',
    'Profile Fields' => 'Настройка полей',
    'Rights' => 'Настройка прав',
	'Calls' => 'Звонки',
    'Company settings' => 'Настройки компании',
	'Base settings' => 'Общие настройки',
    'Jobs' => 'Виды работ',
    'Statuses' => 'Статусы проектов',
    'Templates Messages' => 'Шаблоны сообщений',
    'Login' => 'Вход',
    'Logout' => 'Выход',
    'About' => 'О нас',
    'Contact' => 'Контакты',
    'Operations' => 'Операции',
    'Update' => 'Редактировать',
    'List Categories' => 'Список категорий',
    'All rights reserved.' => 'Все права защищены.',
    'Manage' => 'Управление',
    'You may optionally enter a comparison operator (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b> or <b>=</b>) at the beginning of each of your search values to specify how the comparison should be done.'=>'Вы можете использовать операторы сравнения (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b> or <b>=</b>) установив их перед значением.',
    'Cat Name' => 'Наименование',
    'Parent' => 'Родитель',
    'Advanced Search' => 'Расширенный поиск',
    'Create' => 'Добавить',
    'View Categories' => 'Просмотреть категорию',
    'Search' => 'Искать',
    'Save' => 'Сохранить',
    'Fields with <span class="required">*</span> are required.' => 'Поля с <span class="required">*</span> обязательны.',
    'Cat' => 'Категория',
    'Job Name' => 'Вид работ',
    'Create Jobs' => 'Добавить работу',
    'List Jobs' => 'Список видов работ',
    'Manage Jobs' => 'Управлять списком',
    'View Jobs' => 'Просмотреть категорию',
    'Delete Jobs' => 'Удалить работу',
    'Update Jobs' => 'Редактировать работу',
    'Name' => 'Имя переменной',
    'Title' => 'Наименование',
    'Text' => 'Текст',
    'Type' => 'Тип',
    'Status' => 'Статус',
    'Project Statuses' => 'Статусы проектов',
	'Parts statuses' => 'Статусы этапов',
    'List ProjectStatus' => 'Список статусов',
    'Create ProjectStatus' => 'Добавить статус',
    'Manage ProjectStatus' => 'Управлять списком',
    'View ProjectStatus' => 'Просмотреть статус',
    'Templates' => 'Шаблоны',
    'List Templates' => 'Список шаблонов',
    'Manage Templates' => 'Управление шаблонами',
    'Create Templates' => 'Добавить шаблон',
    'Select' => 'Выберите',
    'Delete Categories' => 'Удалить категорию',
    'Are you sure you want to delete this item?' => 'Вы уверены что хотите удалить это?',
    'No' => 'Нет',
    'Update ProjectStatus' => 'Редактировать статус',
    'Delete ProjectStatus' => 'Удалить статус',
    'Author' => 'Исполнитель',
    'Customer' => 'Заказчик',
    'For Customer' => 'Заказчикy',
    'Service' => 'Сервис',
	'Hint for manager' => 'Подсказка для администратора',
    'Button in chat for author' => 'Шаблоны для кнопок в чате исполнителя',
    'Button in chat for guest' => 'Шаблон для кнопок в заказе гостя',
	'Hint for customer when he sending a message' => 'Всплывающая подсказка при отправлении сообщения заказчиком',
    'Update Templates' => 'Редактировать шаблон',
    'Delete Templates' => 'Удалить шаблон',
    'View Templates' => 'Просмотреть шаблон',
    'Submit' => 'Отправвить',
    'Order' => 'Заказ',
    'All orders' => 'Все заказы',
    'Create order' => 'Создать заказ',
    'Nam' => 'Имя',
    'Subject' => 'Тема',
    'Body' => 'Текст',
    'Email' => 'Email',
    'Verification Code' => 'Код проверки',
    'Captcha' => 'Каптча',
    'Please enter the letters as they are shown in the image above.' => 'Пожалуйста, введите буквы как на картинке выше.',
	'Please fill out the following form with your login credentials:'=> 'Пожалуйста, заполните форму в соответствии с Вашими учётными данными:',
    'Letters are not case-sensitive.' => 'Регистр не имеет значения.',
    'Upload Files' => 'Загрузить файлы',
    'click here' => 'жмите сюда',
    'Uploading your file(s), please wait...' => 'Подождите, идет загрузка...',
    'Select File(s)' => 'Выбрать файлы',
    'Delete Selected File(s)' => 'Удалить выбранные',
	'Tip' => 'Совет менеджеру',
    'Close Window' => 'Закрыть окно',
    'cancel the upload' => 'отменить загрузку',
    'Upload' => 'Загрузить',
    'Select a Background image' => 'Чтобы выбрать файлы',
    'Update Categories' => 'Редактировать категорию',
    'Message' => 'Сообщения',
    'Inbox' => 'Входящие',
    'Sent' => 'Исходящие',
    'Trash' => 'Корзина',
    'Personal area'=>'Личный кабинет',
    'Personal account'=>'Личный счет',
    'Profile'=>'Профиль',
	'My profile' => 'Мой профиль',
    'Orders'=>'Заказы',
    'New projects'=>'Новые проекты',
    'New projects for technical'=>'Проекты для технических руководителей',
    'NEW ORDERS'=>'Новые заказы',
    'My orders'=>'Мои заказы',
    'Create order'=>'Создать заказ',
    'Order statuses'=>'Статусы заказов',
    'Events' => 'События',
	'Events for manager' => 'События для администратора',
	'Events for sales-manager' => 'События для менеджера',
	'Link to order page' => 'Ссылка на страницу заказа',
	'You have received a new message.' => 'Вам пришло новое сообщение.',
    'Confirm' => 'Подтвердить',
    'Confirmed' => 'Подтвержден',
    'Delete' => 'Удалить',
    'Show' => 'Показать',
	'Saving...' => 'Сохраняем...',
	'Click to edit' => 'Кликните, чтобы редактировать',
	'Reject' => 'Отклонить',
	'Rejected' => 'Отклонен',
	'Cancel' => 'Отмена',
	'Payment method' => 'Способ оплаты',
	'New' => 'Новые',
    'Logs' => 'История действий',
	'Managers logs' => 'История действий администраторов',
    'Bookkeeping' => 'Бухгалтерия',
    'My account' => 'Личный кабинет',
    'Profile' => 'Профиль',
    'Authors' => 'Авторы',
    'Customers' => 'Заказчики',
    'Accounts department' => 'Бухгалтерия',
	'Project Fields' => 'Поля Проекта',
	'Project fields settings' => 'Настройка полей проекта',
	'Profile fields settings' => 'Настройка полей профиля',
	//Company
	'List companies' => 'Список компаний',
	'Create company' => 'Создать компанию',
	'Edit company' => 'Редактировать компанию',
	'company' => 'компания',
	'company name' => 'название компании',
	'frozen' => 'заморожена',
	'domains' => 'доменные имена компании (например company.admintrix.com; если их несколько - пишем через пробел)',
	'language' => 'язык интерфейса',
	'support email' => 'электронная почта поддержки',
	'cash' => 'наличные',
	'2Checkout id' => 'Уникальный идентификатор 2Checkout(если подключается платежная система 2checkout)',
	'2checkout hash' => 'Хэш 2Checkout',
	'front page url' => 'адрес главной страницы',
	'icon' => 'фавикон',
	'logo' => 'лого',
	'header text' => 'текст в шапке',
	'text for guests' => 'текст в шапке, который показывается не зарегистрированным исполнителям',
	'text for customers' => 'текст для заказчиков',
	'agreement for customers' => 'пользовательское соглашение для заказчиков',
	'agreement for executors' => 'пользовательское соглашение для исполнителей',
	'webmaster first customer order rate' => 'процент который выплачивается партнёру,  который привел клиента (вебмастеру)',
	'webmaster second customer order rate' => 'процент(множитель) вэбмастеру за повторные заказы от заказчика',
	'webmaster first executor order rate' => 'процент(множитель) вэбмастеру за первый заказ от исполнителя',
	'webmaster second executor order rate' => 'процент(множитель) вэбмастеру за повторные заказы от исполнителя',
	'telfin id' => 'идентификатор в системе telfin  (при подключении телефонии от telfin.ru - уточните свой ключ в своём личном кабинете telfin.ru)',
	'telfin secret' => 'Секретный код в системе Telfin',
	'User Agreement' => 'Пользовательское соглашение',
	'Agreement' => 'Cоглашение',
	'Variable name' => 'Имя переменной',
	'Tree structure company' => 'будет ли структура проектов в виде дерева (подзаказы)',
	'Paste these links on your site' => 'Вставьте эти ссылки у себя на сайт',
	'Creation of the application by the customer with the completion of the brief' => 'Создание заявки заказчиком с заполнением брифа',
	'Personal cabinet (authorization; for any type of user)' => 'Личный кабинет (авторизация; для любого типа пользователя)',
	'Quick registration (or, also, registration of the customer)' => 'Быстрая регистрация (или, так же регистрация Заказчика)',
	'Registering the company as WEB MASTER (Partner, leading clients)' => 'Регистрация в компании как WEB MASTER (Партнер, приводящий клиентов)',
	'List of orders for freelancers (not yet registered potential performers)' => 'Список заказов для фрилансеров (ещё не зарегистрированных потенциальных исполнителей)',
	'Quick order' => 'Быстрый заказ',
	'Contact details' => 'контактные данные',
	// работа с этапами 
    'StepStatuses' => 'Статусы этапов',
    'Part Statuses' =>  'Состояние этапов',
    'List PartStatus' =>  'Список состояний этапов',
	'Create PartStatus' =>  'Создать статус',
	'Manage PartStatus' =>  'Управление состоянием этапов',
	'Update PartStatus' => 'Сохранить',
	'Delete PartStatus' => 'Удалить',
	'View PartStatus' => 'Показать списком',
	'AfterModerate'  => 'Ваши изменения приняты и вступят в силу после модерации.',
	'YouAreExecutor'  => 'Вы назначены исполнителем данного заказа',
	'The requested page does not exist.' => 'Приносим свои извинения, данной страницы не существует.',
	'Approve'  => 'Одобрить',
	'Reject'  => 'Отклонить',
	'Changed' => 'Изменен',
	'Message from' => 'Сообщение от',
	'You may change the content of this page by modifying the following two files:'=>'Вы можете изменить содержимое этой страницы посредством изменения следующих файлов:',
	'View file' => 'Файл представления',
	'Layout file' => 'Файл шаблона',
	'Attribute' => 'Атрибут',
	'Old value' => 'Старое значение',
	'New value' => 'Новое значение',
	'Date of change' => 'Дата изменения',
	'Good day' => 'Добрый день',
	'We invite You to project' => 'Приглашаем в проект',
	'Question category' => 'Категория вопроса',
	'Question' => 'Вопрос',
	'Answer text' => 'Текст ответа',
	'The text of the answer is above in the upper left of the field for input' => 'Текст ответа - выше слева вверху поля для ввода',
	'Contact with us' => 'Связаться с нами',
	'If You have suggestions or questions, please fill out the form below. Thank you' => 'Если у Вас есть предложения или вопросы, пожалуйста, заполните форму ниже. Спасибо.',
	'Error' => 'Ошибка',
	'Projects statuses' => 'Статусы заказов',
	'Send SMS' => 'Отправить смс',
	'News' => 'Новости',
	'With selected' => 'С выбранными',
	'Last Updated'=> 'Дата последнего изменения',
	'Site News' => 'Новости сайта', 
	'Check All' => 'Выбрать все',
	'UnCheck All' => 'Сбросить все',
	'Link is absent' => 'Ссылка отсутствует',
	'Tree structure' => 'Древовидная структура',
	'Orders treeview' => 'Мои заказы в виде дерева',
	'smsc.ru api login' => 'smsc.ru (сервис для отправки смс) api логин',
	'smsc.ru api password' => 'smsc.ru (сервис для отправки смс) api пароль',
	//sms
    'Invalid parameters' => 'Ошибка в параметрах',
    'Invalid login or password' => 'Неверный логин или пароль',
    'Insufficient funds in the account' => 'Недостаточно средств на счету Клиента',
    'IP-address temporarily blocked due to frequent errors in querie' => 'IP-адрес временно заблокирован из-за частых ошибок в запросах',
    'Invalid date' => 'Неверный формат даты',
    'Message is prohibited (by text or by the name of the sender)' => '',
    'Invalid number phone' => 'Неверный формат номера телефона',
    'Message to the specified number can not be delivered' => 'Сообщение на указанный номер не может быть доставлено',
    'Sending more than one of the same request to send SMS-messages within minutes' => 'Отправка более одного одинакового запроса на передачу SMS-сообщения в течение минуты',

	'From' => 'От',
	'To' => 'Кому',
	'Body' => 'Тело',
	'Type' => 'Тип',
	'Date' => 'Дата',
	'Delivery' => 'Рассылка',
	'SMTP-servers' => 'SMTP-сервера',
    'Address' => 'Адрес',
    'Port' => 'Порт',
    'Encryption' => 'Шифрование',
    'Username' => 'Имя пользователя',
    'Password' => 'Пароль',
    'Letters per hour' => 'Писем в час',
    'Start of current range' => 'Начало текущего диапозона',
    'Letters sent' => 'Писем отправлено',
    'Manage SMTP-servers' => 'Управление SMTP-серверами',
    'Add SMTP-server' => 'Добавить SMTP-сервер',
    'Update SMTP-server' => 'Обновить SMTP-сервер',
	'on' => 'включено',
	'off' => 'выключено',
	// Emails
	'Notification' => 'Уведомление',

	//Logs
	'Date and time' => 'Дата и время',
	'Manager' => 'Администратор',
	'Sales manager' => 'Менеджер',
	'Action' => 'Действие',
	'Order number' => 'Номер заказа',
	'Number of user actions: {start} - {end} from {count}' => 'Количество действий пользователя: {start}-{end} из {count}',

	'Variables' => 'Переменные',
	'%site%' => '%сайт%',
	'%the link to the password change page%' => '%ссылка на страницу изменения пароля%',
	'%support%' => '%support%',
	'%company%' => '%компания%',
	'%name%' => '%имя%',
	'%Name%' => '%Имя%',
	'%login%' => '%login%',
	'%password%' => '%password%',
	'%the link to the personal account page%' => '%ссылка на страницу личного кабинета%',
	'%Order link%' => '%Ссылка на заказ%',
	'%order link%' => '%ссылка на заказ%',
	'%Job title%' => '%название работы%',
	'%order no%' => '%№ заказа%',
	'%the order number%' => '%номер заказа%',
	'%Job title%' => '%тема работы%',
	'%job title%' => '%тема заказа%',
	'%Title%' => '%Наименование%',
	'%title%' => '%наименование%',
	'%from the cost field%' => '%из поля стоимость%',
	'%the amount of charged%' => '%сумма из выставлено к оплате%',
	'%the amount to be paid%' => '%сумма к оплате%',
	'%message body%' => '%текст сообщения%',
	'%the link to the payment page%' => '%ссылка на страницу оплаты%',
	'%specialization%' => '%специальность%',
	'%the name of the stage%' => '%название части%',
	'%new order%' => '%новый заказ%',

	'Notification for executor about the occurrence timing of the completion of the order' => 'Уведовление о наступлении сроков завершения заказа',
	'Notification for executor about the occurrence timing of the completion of the completion point' => 'Уведовление о наступлении сроков завершения этапа заказа',

	'Templates steps' => 'Шаблоны этапов',
	'Manage Templates steps' => 'Управление шаблонами этапов',
	'Update Templates steps' => 'Редактирование шаблона этапов',
	'Name template' => 'Имя шаблона',
	'Steps' => 'Этапы',
	'Add steps' => 'Добавить этап',
	'You must fill in the name of the step' => 'Необходимо заполнить имя этапа',
	'It is necessary to fill in the stage term correctly' => 'Необходимо коректно заполнить срок этапа',
	'The term of all stages should be equal to 100%' => 'Срок всех этапов должен быть равен 100%',
	'Stage name' => 'Имя этапа',
	'Term of the stage (%)' => 'Срок этапа(%)',
	'Delete step' => 'Удалить этап',
	'Count steps' => 'Количество этапов',
	'Create Templates steps' => 'Создание шаблона этапов',
	'Template saved successfully' => 'Шаблон сохранен успешно',
	'Load from template' => 'Загрузить с шаблона',

	//Templates
	'For answers via chat' => 'Для ответов через чат',
	'Service messages sent automatically to email' => 'Сервисные сообщения',
	'Tips for managers' => 'Подсказки для администраторов',
	'Button templates' => 'Шаблоны кнопок',
	'Different service messages' => 'Разные сервисные сообщения',
	'Category question' => 'Категория  вопроса',
	'The category of the question (for example, "questions on payment" or "questions on the schedule of work", etc.)' => 'Категория  вопроса  (например, “вопросы по оплате” или “вопросы по графику работы “ и т.д.)',
	'Letter subject' => 'Тема письма',
	'message' => 'Сообщение',
	'Field name' => 'Название поля',
	'Button label' => 'Надпись на кнопке',
	'Message text' => 'Текст сообщения',
	'User type' => 'Тип пользователя',
	'Service mail: Password recovery' => 'Восстановление пароля',
	'Service mail: Registration' => 'Успешная регистрация',
	'Service mail: Project accepted' => 'Регистрация проекта',
	'Service mail: payment awayting' => 'Оплата заказа',
	'Service mail: part done' => 'По Вашему заказу появилась готовая часть',
	'Service mail: work done' => 'Работа готова',
	'Service mail: new message for customer' => 'Сообщение в чате',
	'Service mail: order closed' => 'Заказ завершен',
	'Service mail: new project' => 'Появился новый заказ по Вашей специальности',
	'Service mail: executor assigment' => 'О назначении Исполнителем',
	'Service mail: new message for author' => 'Сообщение в чате для исполнителя',
	'Service mail: ouster' => 'Сообщение о съеме с заказа',
	'Service mail: deadline arrived' => 'Срок сдачи части наступил',
	'Service mail: new revision' => 'Сообщение о новой доработке',
	'Service mail: your salary' => 'Сообщение об оплате заказа',
	'Message for an author in response to project' => 'Сообщение для автора при отклике на проект',
	'Notification for correctors' => 'Уведомление для технических руководителей',
	'Message for a customer when the agreement was not accepted' => 'Сообщение для клиента, когда он не согласен с условиями соглашения',
	'Message for a manager that the agreement was not accepted by the customer' => 'Сообщение для администратора, что соглашение не было принято заказчиком',

	'Order status' => 'Cтатус заказа',
	'Notes' => 'Заметки',
	'Complete order' => 'Завершить заказ',
	'information about order' => 'Информация о заказе',
	'Artist search' => 'Поиск исполнителя',
	'Status of part' => 'Статус части',
	'The new part' => 'Новая часть',
	'Term of the stage' => 'Срок этапа',
	'Delete Part' => 'Удалить часть',
	'Add part' => 'Добавить часть',
	'Revisions' => 'Доработки',
	'Date and time of order execution' => 'Дата и время исполнения заказа',
	'Date of informing (manager)' => 'Дата информирования (администратор)',
	'Term for the Author' => 'Срок для Автора',
	'Recipient' => 'Получатель',
	'Send to mail' => 'Отправить на почту',
	'Chat general comment' => 'Чат общий комментарий',
	'Project cost' => 'Стоимость проекта',
	'Issue an invoice:' => 'Выписать счет: ',
	'Received from the client:' => 'Получено от клиента: ',
	'Worth on payment:' => 'Стоит на оплате: ',
	'Cost for the author:' => 'Стоимость для автора: ',
	'Paid work:' => 'Оплачено работы: ',
	'Send to payment' => 'Отправить на оплату',
	'Total for payment:' => 'Суммарно на оплату: ',
	'Confirm button' => 'Кнопка подтвердить',
	'Cancel button' => 'Кнопка отмена',
	'Link for freelancer' => 'Ссылка для фрилансера',
	'Return' => 'Возврат',
	'Return an order' => 'Вернуть заказ',

	'Parent management' => 'Управление родительскими категориями',
	'Childs management' => 'Управление дочерними категориями',
	'Create Parents Categories' => 'Создать родительскую категорию',

	'Salaries' => 'Зарплаты',
	'Manage classifying actions' => 'Управление классификациями действий',
	'Select class an employee...' => 'Выберите класс сотрудника...',
	'Select an employee...' => 'Выберите сотрудника...',
	'calculation' => 'Расчет',
	'Award' => 'Премию/штраф',
	'Accrue' => 'Начислить',
	'Accrue salary' => 'Начислить зарплату',
	'Payment for actions' => 'Оплата за действия',
	'Create actions' => 'Создать действие',
	'Calculate the salary of an employee' => 'Начислить зарплату сотруднику',
	'Delete actions' => 'Удалить действие',
	'Manage actions' => 'Управление действиями',
	'Create classifying actions' => 'Создать классификацию действия',
	'Update classifying actions' => 'Редактирование классификации действия',
	'View manager order' => 'Просмотр заказа менеджером',
	'Checking the manager order' => 'Проверка заказа менеджером',

	'Filter templates' => 'Шаблоны фильтров',
	'Filters' => 'Фильтры',
	'Create filter' => 'Создать фильтр',
	'Delete filter' => 'Удалить фильтр',
	'Manage filter' => 'Управление фильтрами',
	'Name filter' => 'Название фильтра',
	'For table' => 'Для таблицы',
	'Filter' => 'Фильтр',
	'By default' => 'По умолчанию',
	'For role' => 'Для роли',
	'Column' => 'Поле',
	'Operator' => 'Оператор',
	'Value' => 'Значение',
	'Select an role...' => 'Выберите роль...',
	'Select an table...' => 'Выберите таблицу...',

	/*table column*/
	/*user*/
	'id' => 'ИД',
	'username' => 'Логин',
	'email' => 'Email',
	'create_at' => 'Создан',
	'lastvisit_at' => 'Последний визит',
	'superuser' => 'Супер-админ',
	'status' => 'Статус',
	'identity' => 'identity',
	'network' => 'network',
	'full_name' => 'Полное имя',
	'state' => 'Страна',
	'phone_number' => 'Номер телефона',
	'pid' => 'pid',

	/*Projects*/
	'user_id' => 'Заказчик',
	'title' => 'Заголовок',
	'add_demands' => 'add_demands',
	'last_spam' => 'last_spam',
	'executor' => 'Исполнитель',
	'notes' => 'Заметки',
	'date' => 'Дата создания',
	'max_exec_date' => 'Дата завершения',
	'manager_informed' => 'Сообщение менеджеру',
	'author_informed' => 'Сообщение автора',
	'author_notes' => 'Заметки автора',
	'description' => 'Описание',
	'old_status' => 'old_status',
	'soderjanie' => 'Содержание',
	'specials' => 'Специальность',
	'is_active' => 'Активный',
	'technicalspec' => 'Действие для менеджера',
	'opisanie' => 'Описание',
	'result_form' => 'В каком виде выдавать результат работы?',
	'tech' => 'Технологии используемые в задаче',
	'Dop_fromMan' => 'Особенности задачи',
	'time_to_connect' => 'Время для связи',
	'parent_id' => 'Родительский заказ',
	'executor_event' => 'Событие исполнителя',
	'customer_event' => 'Событие менеджера',
	'text' => 'Текст',
	'pages' => 'Страницы',
	'time_for_call' => 'Время звонка',
	'edu_dep' => 'edu_dep',
    'avtoritet' => 'Заказчик - авторитет',
    'specials2' => 'Тип работы',


	/*Payment*/
	'order_id' => 'Заказ',
	'receive_date' => 'Дата поступления',
	'pay_date' => 'Дата оплаты',
	'theme' => 'Тема',
	'manager' => 'Админ',
	'user' => 'Пользователь',
	'summ' => 'Сумма',
	'details_type' => 'details_type',
	'details_number' => 'details_number',
	'payment_type' => 'Тип оплаты',
	'approve' => 'Проведен',
	'method' => 'Метод',
	
	'my_value' => 'Свое значение',
	'Reset filter' => 'Сбросить фильтр',
	'make financial analysis available' => 'сделать финансовый анализ доступным',
	'Continue' => 'Продолжить',

	'Settings' => 'Настройки',
	'Profile settings' => 'Настройки профиля',
	'Time Work' => 'Время работы',
	'to' => 'до',
	'minutes' => 'минут',
	'at the expiration of' => 'по истечению',
	'Notify about exceeding the expectation from corrector by email' => 'Уведомлять о превышении ожидания от тех рука по email',
	'Notify about exceeding the expectation from corrector by sms' => 'Уведомлять о превышении ожидания от тех рука по sms',
	'Notify about exceeding the expectation from admin by email' => 'Уведомлять о превышении ожидания от админа по email',
	'Notify about exceeding the expectation from admin by sms' => 'Уведомлять о превышении ожидания от админа по sms',
	'Notify about exceeding the expectation from manager by email' => 'Уведомлять о превышении ожидания от менеджера по email',
	'Notify about exceeding the expectation from manager by sms' => 'Уведомлять о превышении ожидания от менеджера по sms',
	'Notice about exceeding the expectation from admin' => 'Уведомление о превышении ожидания от администратора',
	'Notice about exceeding the expectation from manager' => 'Уведомление о превышении ожидания от менеджера',
	'Notice about exceeding the expectation from corrector' => 'Уведомление о превышении ожидания от корректора',
);
