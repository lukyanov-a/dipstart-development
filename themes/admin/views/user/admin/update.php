...
<h1><?php echo UserModule::t('Update User')." "; ?><span id="user_id"><?=$model->id?></span></h1>
			...
			<?php $this->widget('zii.widgets.grid.CGridView', array(
				...
				'columns'=>array(
					array(
						...
						'value'=>'User::itemAlias(\'roles\', $data->getName())',
					),
					array(
						'name'=>'type',
...
<?php
	if ($model->isSalesManager() || $model->isAdmin()) {
		?>
		<hr>
        <div class="manager-informed form text-center">
	...
	if($model->isCustomer(true) ) {
		echo $this->renderPartial('_orderList', array(
				'model'		=> $model,
			)
		);
	}
